...
async function main() {
  ...
  const categories: { name: string; area: BusinessArea }[] = [
  ...
  type SeedProduct = [string, number, boolean?, number?];
  ...
  for (const [categoryName, products] of Object.entries(catalog)) {
    ...
    for (const [index, [name, sellingPrice, track = false, stock = 0]] of products.entries()) {
