...
export function PosRegister({
  ...
  function renderCart() {
    ...
    <>
      <div className="flex flex-wrap items-center gap-3 border-b p-5">
        <ShoppingCart className="text-[#FFD758]" />
        <h1 className="text-xl font-semibold">Current order</h1>
        <span className="ml-auto rounded-md bg-[#FFD758] px-3 py-1 text-sm font-semibold text-black">{itemCount}</span>
        {cart.length > 0 && (
          <button
            onClick={() => setCart([])}
            className="bz-btn-outline min-h-11 px-3 text-sm"
          >
            Clear
          </button>
        )}
      ...
      <div className="min-h-32 flex-1 space-y-3 overflow-y-auto p-4">
        {cart.length === 0 && <p className="py-10 text-center text-sm text-black">Tap a menu item to begin.</p>}
        {cart.map((line) => (
          <div key={line.variantId} className="rounded-md border border-black p-3">
            <div className="flex justify-between gap-3">
              <div>
                <p className="font-medium">{line.productName}</p>
                {line.variantName !=="Portion" && (
                  <p className="text-xs font-semibold text-black">{line.variantName}</p>
                )}
              </div>
              <button
              ...
              >
                <Trash2 size={17} className="text-black" />
              </button>
            </div>
            <div className="mt-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <button className="grid size-11 place-items-center rounded-md border border-black" onClick={() => changeQuantity(line.variantId, -1)}>
                  <Minus size={16} />
                </button>
      ...
      <div className="border-t p-5">
        ...
        {error && !paymentOpen && <p className="bz-alert mt-3">{error}</p>}
        <button
          type="button"
          onClick={() => {
            setError("");
          ...
          className="bz-btn-primary mt-4 min-h-14 w-full text-lg disabled:border-2 disabled:border-dashed"
        >
  ...
  return (
    <div className="grid min-h-[calc(100vh-5rem)] gap-5 pb-24 xl:grid-cols-[1fr_400px] xl:pb-0">
      <section className="min-w-0">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row">
          <label className="relative flex-1">
            <Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-black" size={20} />
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search menu or SKU"
              className="min-h-12 w-full rounded-md border border-black bg-white pl-10 pr-4 outline-none focus:border-[#FFD758]"
            />
          </label>
          <select
            value={category}
            onChange={(event) => setCategory(event.target.value)}
            className="min-h-12 rounded-md border border-black bg-white px-4 font-semibold"
          >
        ...
        {filtered.length === 0 ? (
          <div className="rounded-lg border border-dashed border-black bg-white p-12 text-center">
            ...
            <p className="mt-1 text-sm text-black">Try another name, SKU, or category.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((product) => {
              ...
              return (
                <article
                  key={product.id}
                  className={`rounded-lg border p-4  ${stockTone ||"border-black bg-white"}`}
                >
                  <p className="line-clamp-2 font-medium">{product.name}</p>
                  {product.trackInventory && (
                    <p className={`mt-1 text-xs font-medium ${product.stockQuantity <= 0 ?"text-black" :"text-black"}`}>
                  ...
                  <div className="mt-3 space-y-2">
                    {product.variants.map((variant) => (
                      <button
                        ...
                        className={`flex min-h-11 w-full items-center justify-between gap-2 rounded-md border px-3 text-left font-medium hover:border-[#FFD758] ${
                          single ?"border-black bg-white" :"border-black bg-white"
                        }`}
                      >
      ...
      <aside className="hidden max-h-[calc(100vh-5rem)] flex-col rounded-lg border border-black bg-white  xl:sticky xl:top-8 xl:flex">
        {renderCart()}
      </aside>
      ...
      <button
        ...
        className="fixed inset-x-4 bottom-4 z-40 flex min-h-14 items-center justify-between rounded-md bg-[#FFD758] px-5 font-semibold text-black xl:hidden"
      >
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-50 xl:hidden">
          <button className="absolute inset-0 bg-black" aria-label="Close order" onClick={() => setCartOpen(false)} />
          <div className="absolute inset-x-0 bottom-0 flex max-h-[88vh] flex-col rounded-t-md bg-white border border-black">
            <div className="flex justify-end p-2">
              <button aria-label="Close order" onClick={() => setCartOpen(false)} className="grid size-11 place-items-center rounded-md hover:border hover:border-black">
                <X />
              </button>
            </div>
      ...
      {paymentOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black p-4">
          <div className="w-full max-w-lg rounded-md bg-white p-6 border border-black">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-wider text-black">Total</p>
                <p className="text-3xl font-semibold">{formatMoney(total, currency, 0)}</p>
              </div>
              <button
                aria-label="Close payment"
                onClick={() => setPaymentOpen(false)}
                className="grid size-11 place-items-center rounded-md hover:border hover:border-black"
              >
                <X />
              </button>
            </div>
            ...
            <div className="mt-2 grid grid-cols-3 gap-2">
              {[
                ...
                <button
                  ...
                  className={`min-h-14 rounded-md border px-2 font-medium ${
                    paymentMethod === value ?"border-black bg-[#FFD758] text-black" :"border-black bg-white text-black"
                  }`}
                >
                  {label}
                </button>
            ...
            {paymentMethod ==="CASH" && (
              <div className="mt-5">
                ...
                <input
                  ...
                  inputMode="decimal"
                  autoFocus
                  className="mt-2 min-h-12 w-full rounded-md border border-black px-4 text-lg font-bold"
                />
                <div className="mt-3 flex justify-between rounded-md bg-white p-3">
                  <span>Change</span>
            ...
            <button
              ...
              className="bz-btn-primary mt-6 min-h-14 w-full text-lg disabled:border-2 disabled:border-dashed"
            >
      ...
      {completed && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black p-4">
          <div className="w-full max-w-md rounded-md bg-white p-8 text-center border border-black">
            <CheckCircle2 className="mx-auto text-[#FFD758]" size={56} />
            <h2 className="mt-4 text-2xl font-semibold">Sale completed</h2>
            <p className="mt-2 font-mono text-sm">{completed.receiptNumber}</p>
            <p className="mt-5 text-3xl font-semibold">{formatMoney(Number(completed.total), currency, 0)}</p>
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              ...
              <button onClick={newSale} className="bz-btn-primary min-h-12">
                New sale
              </button>
            </div>
          </div>
