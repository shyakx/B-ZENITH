...
export function ReturnForm({ saleId, items }: { saleId: string; items: Item[] }) {
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        {items.map((item) => (
          <label key={item.id} className="grid items-center gap-2 rounded-md border p-3 sm:grid-cols-[1fr_auto]">
            <span><b>{item.name}</b><span className="block text-sm text-black">{Number(item.unitPrice).toLocaleString("en-RW")} RWF · {item.available} returnable</span></span>
      ...
      <div className="flex justify-between text-xl font-semibold"><span>Return total</span><span>{total.toLocaleString("en-RW")} RWF</span></div>
      {message && <p className={message.startsWith("Return RET") ?"bz-success" :"bz-alert"}>{message}</p>}
      <button onClick={submit} disabled={pending || !selected.length || reason.trim().length < 3} className="bz-btn-primary w-full disabled:border-2 disabled:border-dashed">{pending ?"Processing…" :"Process return"}</button>
    </div>
  );
}
