  ...
  if (step === "confirm" && product && countedQuantity !== null && adjustment !== null) {
    return (
      <div className="space-y-4">
        <p className="text-sm font-medium text-stone-600">System says {formatQuantity(currentQuantity, unit)}</p>
        <p className="text-sm font-medium text-stone-600">You counted {formatQuantity(countedQuantity, unit)}</p>
        <p className="text-lg font-black">Difference: {differenceLabel}</p>
        <p className="text-sm font-medium text-stone-700">
          {locationLabel(locationCode)} · {product.name}
        </p>
        <p className="text-lg font-black">Save this count?</p>
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <article className="rounded-md bg-stone-50 p-3">
          <p className="text-xs font-bold uppercase tracking-widest text-stone-500">System says</p>
          <p className="mt-1 text-2xl font-black">{formatQuantity(currentQuantity, unit)}</p>
        </article>
      ...
      {message ? (
        <p className={`text-sm font-bold ${message.startsWith("Saved count") ? "text-emerald-700" : "text-red-700"}`}>
          {message}
        </p>
      ) : null}
      <button
