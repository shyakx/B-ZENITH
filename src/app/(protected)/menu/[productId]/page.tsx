...
export default async function EditProductPage({
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      <div><Link href="/menu" className="text-sm font-bold text-black">← Back to menu</Link><h1 className="mt-2 text-3xl font-semibold">Edit {product.name}</h1></div>
      <form action={updateProduct.bind(null, product.id)} className="grid gap-4 rounded-lg border bg-white p-5 md:grid-cols-2">
        ...
        <label className="text-sm font-bold">Cost price<input required name="costPrice" type="number" min="0" step="0.01" defaultValue={product.costPrice.toFixed(2)} readOnly={!canPrice} className="mt-1 min-h-11 w-full rounded-md border px-3 font-normal read-only:bg-white" /></label>
        <label className="text-sm font-bold">Selling price<input required name="sellingPrice" type="number" min="0.01" step="0.01" defaultValue={product.sellingPrice.toFixed(2)} readOnly={!canPrice} className="mt-1 min-h-11 w-full rounded-md border px-3 font-normal read-only:bg-white" /></label>
        {!canPrice ? (
          <p className="text-sm text-black md:col-span-2">Only a manager can change cost or selling prices.</p>
        ) : null}
        <div className="text-sm">
          <p className="font-bold">Available</p>
          <p className="mt-1 min-h-11 rounded-md border bg-white px-3 py-3 font-normal">
            {product.stockQuantity} {product.unit.toLowerCase()}
          </p>
          <Link href="/inventory#stock-take" className="mt-1 inline-block font-bold text-black">
        ...
        <button className="bz-btn-primary md:col-span-2">Save changes</button>
      </form>
      {canDelete ? (
      ...
      {product.variants.length > 0 && (
        <section className="rounded-lg border bg-white p-5">
          <h2 className="text-lg font-semibold">Selling units</h2>
          <div className="mt-3 divide-y">
            {product.variants.map((variant) => (
              <div key={variant.id} className="flex justify-between py-3">
                <div>
                  <b>{variant.name}</b>
                  <p className="text-xs text-black">{variant.sku}</p>
                </div>
                <b className="text-black">{variant.sellingPrice.toFixed(0)} RWF</b>
              </div>
            ))}
          </div>
        </section>
