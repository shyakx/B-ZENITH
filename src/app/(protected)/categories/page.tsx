...
export default async function CategoriesPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div><p className="text-sm font-bold uppercase tracking-widest text-black">Menu</p><h1 className="text-3xl font-semibold">Categories</h1></div>
      <form action={createCategory} className="flex gap-3 rounded-lg border bg-white p-4">
        ...
        <button className="bz-btn-primary">Add</button>
      </form>
      <div className="space-y-3">
        {categories.map((category) => (
          <form key={category.id} action={updateCategory.bind(null, category.id)} className="grid items-center gap-3 rounded-lg border bg-white p-4 sm:grid-cols-[1fr_auto_auto]">
            <div><input required name="name" defaultValue={category.name} className="min-h-11 w-full rounded-md border px-3 font-bold" /><p className="mt-1 text-xs text-black">{category._count.products} products</p></div>
