...
export default async function SuppliersPage() {
  ...
  return (
    <div className="space-y-6">
      <div><p className="text-sm font-bold uppercase tracking-widest text-black">Purchasing</p><h1 className="text-3xl font-semibold">Suppliers</h1></div>
      <details className="rounded-lg border bg-white"><summary className="min-h-14 cursor-pointer p-4 font-semibold">Add supplier</summary><form action={createSupplier} className="grid gap-3 border-t p-4 md:grid-cols-2"><Fields /><button className="bz-btn-primary md:col-span-2">Create supplier</button></form></details>
      <div className="grid gap-3 xl:grid-cols-2">
        ...
        {suppliers.length === 0 && <p className="rounded-lg border border-dashed bg-white p-10 text-center text-black">No suppliers yet.</p>}
      </div>
    </div>
  );
}
