...
import { processHandover, requireOperableSession } from "@/lib/hospitality-service";
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid handover request." }, { status: 400 });
  }
  ...
  try {
    ...
    await processHandover(parsed.data.sessionId, parsed.data.newWaiterId, manager.id, parsed.data.reason);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return hospitalityResponse(error, "Unable to process handover.");
  }
}
